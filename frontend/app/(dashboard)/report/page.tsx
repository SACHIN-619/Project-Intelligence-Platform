...
export default function ReportPage() {
  ...
  async function handleDownload(id: string) {
    ...
    try {
      const token = localStorage.getItem('pii_access_token')
      ...
      if (!res.ok) {
        ...
        throw new Error(err.message || 'Unauthorized or expired session')
      }
      const blob = await res.blob()
      const blobUrl = window.URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = blobUrl
      a.download = `Project_Report_${activeProject?.name || id}.pdf`
  ...
  async function handleGenerate() {
    ...
    const { data, error } = await generateReport({
      ...
      include_monte_carlo: sections.find(s => s.id === 'monte_carlo')?.checked,
      include_evidence: sections.find(s => s.id === 'audit_trail')?.checked,
  ...
  return (
    <div className="p-4 lg:p-6">
      <div className="mb-5">
        <h1 className="text-xl font-bold text-white lg:text-2xl">Report Builder</h1>
        <p className="mt-0.5 text-sm text-[#9CA3AF]">Customise and download your project intelligence report</p>
      </div>
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[380px_1fr]">
        <div className="flex flex-col gap-4">
          <div className="card-panel p-4">
            <p className="text-xs font-semibold uppercase tracking-wider text-[#9CA3AF]">Project</p>
            <div className="mt-2 flex items-center gap-2 rounded-md border border-[#374151] bg-[#0A0F1E] px-3 py-2.5">
              <span className="material-symbols-outlined text-base text-[#3B82F6]">folder_open</span>
                ...
                {scenarios.slice(0,5).map(s => (
                  <label key={s.scenario_id} className="flex cursor-pointer items-center gap-3 rounded-md px-2 py-2 hover:bg-[#1F2937]">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[#9CA3AF] mt-0.5">+{s.days_saved} days saved</p>
                    </div>
          ...
          <div className="card-panel p-4">
            ...
            <div className="flex flex-col gap-1.5 text-xs">
              {[
                ['Sections selected', `${checkedCount} of ${sections.length}`],
                ['Scenarios included', `${selectedScenarios.size} of ${scenarios.length}`],
                ['Estimated pages', `~${estimatedPages}`],
                ['AI narrative', checkedCount > 0 ? '✓ Included' : '—']
              ].map(([l, v]) => (
                <div key={l} className="flex items-center justify-between py-0.5">
                  <span className="text-[#9CA3AF] text-sm">{l}</span>
                  <span className={cn('font-semibold text-sm', v === '✓ Included' ? 'text-[#22C55E]' : 'text-white')}>{v}</span>
                </div>
              ))}
            </div>
          </div>
          <button onClick={() => setShowPreview(v => !v)} className="btn-ghost w-full justify-center text-sm lg:hidden">
          ...
          <button onClick={handleGenerate} disabled={isGenerating || checkedCount === 0} className="btn-primary w-full justify-center">
            {isGenerating ? (
              <><span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white" />Generating report...</>
            ) : (
              <><span className="material-symbols-outlined text-base">download</span>Generate &amp; Download Report</>
            )}
          </button>
          {isGenerating && (
            <div>
              <ProgressBar value={60} colour="#3B82F6" className="mt-2" height={4} />
              <p className="mt-1.5 text-center text-xs text-[#9CA3AF]">Building AI narrative...</p>
            </div>
          )}
          {lastGenerated && (
            <p className="text-center text-xs text-[#9CA3AF] mt-2">
              ...
              {reportId && (
                <>{' · '}<button onClick={() => handleDownload(reportId)} className="text-[#3B82F6] hover:underline font-semibold">Download again</button></>
              )}
            </p>
          )}
        </div>
        <div className={cn('hidden lg:block', showPreview && '!block')}>
          <div className="sticky top-4">
            <div className="mb-2 flex items-center justify-between">
              <h3 className="text-sm font-semibold uppercase tracking-wider text-[#9CA3AF]">Preview</h3>
              <span className="text-xs text-[#9CA3AF]">Updates as you select sections</span>
            </div>
            <div className="max-h-[calc(100vh-200px)] overflow-y-auto rounded-lg border border-[#1F2937]">
              <div className="bg-[#0C1322] p-6 text-slate-300">
                <div className="mb-6 border-b border-[#1F2937] pb-4">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="flex h-6 w-6 items-center justify-center rounded bg-[#3B82F6]">
                      <span className="text-[10px] font-black text-white">PII</span>
                    </div>
                    <span className="text-xs font-semibold text-slate-400">Project Intelligence Interface</span>
                  </div>
                  <h2 className="text-base font-bold text-white">{activeProject.name}</h2>
                  <p className="text-xs text-slate-500 mt-1">Generated: {new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })} · Confidential</p>
                </div>
                {sections.filter(s => s.checked).map(s => {
                  ...
                  return (
                    <div key={s.id} className="mb-5">
                      ...
                      <div className="h-px bg-[#1F2937] mb-2.5" />
            ...
            <div className="mt-3 flex items-center justify-center gap-1.5">
              {Array.from({ length: Math.min(estimatedPages, 8) }).map((_, i) => (
                <span key={i} className={cn('h-1.5 w-1.5 rounded-full', i === 0 ? 'bg-[#3B82F6]' : 'bg-[#374151]')} />
              ))}
